 ...
 *
 ****************************************************************
 *	This file implements the code for TCP channel in Unix
 *	under Tcl 8.x.
 ****************************************************************
 *
...
Dp_TcpAccept(interp, channelId)
    ...
    CONST84 char *channelId;            /* Name of a server Tcp channel. */
{
...
#ifdef SOCKDEBUG
    {
	char msg[DP_TCP_RECVBUFSIZE + 1];
	if (result > 0) {
	    memcpy(msg, buf, result);
	    msg[result] = '\0';
	    printf("Received %d TCP bytes: %s\n", result, msg);
	}
    }
#endif
